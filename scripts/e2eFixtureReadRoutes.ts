...
export function handleFixtureReadRoute(
	...
	if (url.pathname === API_ROUTES.bootstrap) {
		...
		return fixtureJson({
			...
			voiceCommands: {
				enabled: state.voiceCommandsReady,
				ready: state.voiceCommandsReady,
				state: state.voiceCommandsReady ? "ready" : "disabled",
				model: "Cactus-Compute/needle2",
				reason: state.voiceCommandsReady
					? null
					: "Voice commands are disabled in the browser fixture.",
	...
	if (nestedPath === "files/review") {
		return fixtureJson({ reviews, revision: state.reviewRevision } satisfies ReviewStateResponse);
	}
	...
	if (nestedPath === "events") {
		return eventStream({
			...
			repositoryId,
			operationRevision: state.operationRevision,
			stateRevision: state.reviewRevision,
			catalogRevision: 1,
