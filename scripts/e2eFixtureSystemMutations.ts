...
async function handleFixtureSpeechMutation(
	...
	return fixtureJson({
		text: url.searchParams.get("language") === "en" ? "voice command phrase" : "dictated phrase",
		language: "en",
		durationMs: 300,
		inferenceMs: 4,
	});
...
async function handleFixtureVoiceCommandMutation(
	state: FixtureMutableState,
	request: Request,
	url: URL,
): Promise<Response | null> {
	if (url.pathname === "/api/e2e/voice-commands/enable" && request.method === "POST") {
		state.speechReady = true;
		state.voiceCommandsReady = true;
		state.settingsProfiles = state.settingsProfiles.map((profile) => ({
			...profile,
			data: {
				...profile.data,
				voice: { commandsEnabled: true },
			},
		}));
		return fixtureJson({ ready: true });
	}
	if (url.pathname === "/api/e2e/voice-commands/action" && request.method === "POST") {
		const input = (await request.json()) as { actionId?: unknown; confidence?: unknown };
		if (
			input.actionId !== "navigate.artifacts" &&
			input.actionId !== "navigate.history" &&
			input.actionId !== "file.stage"
		) {
			return fixtureJson({ error: { code: "invalid_action", message: "Invalid action" } }, 400);
		}
		if (
			...
			return fixtureJson(
				{ error: { code: "invalid_confidence", message: "Invalid confidence" } },
				400,
			);
		}
		state.voiceCommandActionId = input.actionId;
		state.voiceCommandConfidence = input.confidence ?? 0.98;
		return fixtureJson({ actionId: input.actionId, confidence: state.voiceCommandConfidence });
	}
	if (url.pathname !== API_ROUTES.voiceCommandResolve || request.method !== "POST") return null;
	if (!state.voiceCommandsReady) {
		return fixtureJson(
			{
				error: {
					code: "voice_commands_unavailable",
					message: "Fixture voice commands are unavailable.",
				},
			},
			409,
		);
	}
	const input = (await request.json()) as { transcript?: unknown };
	return fixtureJson({
		commands:
			input.transcript === "voice command phrase" ? [{ actionId: state.voiceCommandActionId }] : [],
		confidence: state.voiceCommandConfidence,
		inferenceMs: 4,
		model: "Cactus-Compute/needle2",
		reasoning:
			state.voiceCommandActionId === "file.stage"
				? "'voice command phrase' -> stage_current_file"
				: state.voiceCommandActionId === "navigate.history"
					? "'voice command phrase' -> open_git_history"
					: "'voice command phrase' -> open_artifacts",
	});
}
...
export async function handleFixtureSystemMutation(
	...
	const voiceCommandResponse = await handleFixtureVoiceCommandMutation(state, request, url);
