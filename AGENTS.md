...
`src/client/App.tsx` is the client composition root. Stateful use cases and side effects live in `src/client/features/`, rendering lives in `src/client/components/`, and small client utilities live in `src/client/lib/`. `src/server/` holds the Bun CLI/server, Git operations, validation, and review state. Shared API types live in `src/shared/contracts.ts`. Unit tests are co-located as `*.test.ts(x)`, boundary integration tests also live in `tests/integration/`, and Playwright browser tests for mobile and desktop layouts live in `tests/e2e/`. Use `scripts/` for launch and fixture tools and `public/` for static assets. See `ARCHITECTURE.md` for ownership and dependency direction.
...
- `bun run lint` runs the direct Biome 2.5 policy; `bun run format:check` verifies formatting and import organization for repository paths covered by the Biome configuration.
...
- A change is not complete while `bun run check:quality` or another required targeted verification command fails.
...
### Non-disruptive browser verification

Do not launch visible browser windows, Playwright UI/debug mode, simulators, or other test UI
that can take focus during ordinary automated verification unless the user explicitly requests or
approves it. Run Playwright headlessly by default and use retained screenshots, video, and traces
to diagnose failures. Do not pass `--headed`, `--debug`, or `--ui` without that approval.

`scripts/diffScrollBenchmark.ts` intentionally launches headed, hardware-accelerated Chromium and
disables background throttling so its CPU and power results remain comparable with the documented
baselines. Treat `bun run benchmark:diff-scroll` and `bun run benchmark:diff-scroll:verify` as
focus-disrupting commands: ask immediately before running either one, warn that Chromium will
appear and may steal focus, and wait for explicit approval. Do not silently substitute a headless
run for an accepted performance baseline because it is a different measurement environment. Use
unit tests, static checks, builds, and headless Playwright runs during normal iteration.
...
Keep `127.0.0.1` as the application and development default. Use `--host 0.0.0.0` only as an explicit opt-in on a trusted network when another device needs access, because the server exposes repository contents and staging controls. Review state belongs in the XDG data database, never in repository files. Do not commit credentials, `dist/`, or Playwright reports.

## Agent automation

Keep ordinary browser verification headless. Do not launch visible browsers, Playwright UI/debug
mode, or simulators without explicit user approval. The diff-scroll benchmark intentionally uses
headed Chromium and may steal focus, so warn the user and wait for approval before running
`bun run benchmark:diff-scroll` or `bun run benchmark:diff-scroll:verify`. Do not replace accepted
benchmark measurements with headless results because that changes the measurement environment.
