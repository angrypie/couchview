...
test.describe("production PWA", () => {
	...
	test("keeps documents and APIs network-only without prompting over unsaved work", async ({
		...
		try {
			...
			// The commit sheet hides the review region from assistive tech, so
			// assert on the DOM node rather than its accessible role.
			await expect(page.locator('[aria-label="Current file"][role="region"]')).toBeVisible();
