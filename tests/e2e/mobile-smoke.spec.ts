...
test.describe("mobile fixture review", () => {
	...
	test("keeps the diff contained while typography, wrapping, and review navigation work", async ({
		...
		const surface = diff.locator('[data-renderer="legend-list"]');
		const status = diff.getByTestId("diff-surface-status");
		const horizontalScroller = diff.getByTestId("diff-horizontal-scroll");
		await expect(surface).toBeVisible();
		await expect(status).toHaveAttribute("data-token-complete", "true");
		await expect(surface.locator("canvas")).toHaveCount(0);
		expect(await code.locator("[data-line]").count()).toBeGreaterThan(0);
		...
		const displayControls = page.getByLabel("Diff display controls");
		await expect(displayControls.getByText("11px", { exact: true })).toBeVisible();
		const surfaceBeforeLineNumbers = await status.getAttribute("data-generation");
		...
		await expect
			.poll(() => status.getAttribute("data-generation"))
			.not.toBe(surfaceBeforeLineNumbers);
		await expect(code.locator('[data-testid^="new-line-"]').first()).toBeVisible();
		...
		await expect.poll(() => renderedFont().then((font) => font.fontSize)).toBe("24px");

		await expect
			.poll(() =>
				horizontalScroller.evaluate((element) => element.scrollWidth - element.clientWidth),
			)
			.toBeGreaterThan(20);
		...
		await expect(surface).toHaveAttribute("data-line-wrap", "true");
		await expect
			.poll(() =>
				horizontalScroller.evaluate((element) => element.scrollWidth - element.clientWidth),
			)
	...
	test("runs grouped package commands and reconnects to their output", async ({ page }) => {
		...
		// The sheet dismisses with a short exit animation during which the app
		// behind it stays aria-hidden; wait for it to finish before interacting.
