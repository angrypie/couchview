...
This document records the migration from the hosted React DOM product surface to a universal
Expo Router application. The product now renders with React Native on web and iOS; the same native
path targets Android, which remains unverified. Pierre Diff and Ghostty are the only intentional
Expo DOM boundaries. They occupy exactly three DOM islands:

- `src/client/components/diff/diff-dom-view.tsx` — Pierre Diff.
...
Pierre remains a focused Expo DOM component because it requires workers, DOM measurement, and a
shadow-DOM renderer. React Native owns the route, file and hunk controls, loading/error states,
search interactions, and review/staging actions. The DOM boundary receives serializable diff and
display data and reports token clicks and visible-line changes through async actions.

The previous cross-boundary imperative ref and worker-pool hook were removed. The host now sends a
serializable command revision, while prewarming and Pierre worker ownership remain inside the DOM
document.
...
- Review: repository selection, change filtering, Pierre diff, review/stage/bulk stage, commit,
  search, package scripts/runs, command palette, failure details, and pairing overlays use RN UI.
...
- Validate Android with a development build. Android was not exercised during this migration, so
  no Android runtime or layout acceptance claim is made here.
- Consider a native font asset if exact Iosevka parity is required outside the Ghostty renderer;
  React Native currently uses the platform monospace fallback for ordinary text.
- Revisit native terminal chrome only when a native Ghostty-compatible renderer is available; the
  current boundary is deliberately replaceable.
