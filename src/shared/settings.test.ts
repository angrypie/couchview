...
describe("settings profiles and keymaps", () => {
  ...
  test("normalizes shortcut strokes and rejects malformed sequences", () => {
    expect(parseShortcutSequence([{
      key: "K",
      modifiers: ["shift", "mod"],
    }])).toEqual([{ key: "k", modifiers: ["mod", "shift"] }]);
    expect(() => parseShortcutSequence([])).toThrow("one and four");
    expect(() => parseShortcutSequence(new Array(5).fill({ key: "x", modifiers: [] })))
      .toThrow("one and four");
    expect(() => parseShortcutSequence([{ key: "Control", modifiers: [] }]))
      .toThrow("cannot use");
    expect(() => parseShortcutSequence([{ key: "k", modifiers: ["mod", "mod"] }]))
      .toThrow("Duplicate");
  });
  ...
  test("requires the palette opener to begin with a non-shift modifier", () => {
    ...
    expect(paletteShortcutHasRequiredModifier([{ key: "k", modifiers: ["mod"] }]))
      .toBe(true);
    expect(paletteShortcutHasRequiredModifier([
      { key: "g", modifiers: [] },
      { key: "k", modifiers: ["mod"] },
    ])).toBe(false);
  ...
  test("validates the complete persisted document instead of repairing malformed API data", () => {
    ...
    expect(() => parseSettingsProfileData({ ...data, typography: undefined }))
      .toThrow("Typography preferences");
