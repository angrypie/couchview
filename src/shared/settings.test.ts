...
describe("settings profiles and keymaps", () => {
	test("ships complete QWERTY and Dvorak maps with mnemonic bindings unchanged", () => {
		...
		expect(DEFAULT_KEYBINDINGS.dvorak["navigate.history"]).toEqual(
			DEFAULT_KEYBINDINGS.qwerty["navigate.history"],
		);
	});
	...
	test("keeps voice commands disabled when loading a profile saved before voice settings", () => {
