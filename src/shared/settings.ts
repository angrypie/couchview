...
export function shortcutSequenceKey(sequenceValue: ShortcutSequence): string {
  return sequenceValue.map((item) =>
    `${item.modifiers.join("+")}+${item.key}`
  ).join(" ");
}

function sequenceIsPrefix(
  left: ShortcutSequence,
  right: ShortcutSequence,
): boolean {
  if (left.length > right.length) return false;
  return left.every((item, index) =>
    shortcutSequenceKey([item]) === shortcutSequenceKey([right[index]!])
  );
}
...
export function effectiveKeybindings(
  ...
  return Object.fromEntries(COMMAND_IDS.map((commandId) => [
    commandId,
...
export function paletteShortcutHasRequiredModifier(
...
export function parseSettingsProfileData(value: unknown): SettingsProfileData {
  ...
  if (conflicts.length > 0) {
    throw new Error(
      `Shortcut conflict between ${conflicts[0]!.first} and ${conflicts[0]!.second}`,
    );
  }
