...
export class RepositoryReview {
	...
	async setReview(input: SetReviewRequest): Promise<SetReviewResponse> {
		...
		if (
			...
			throw new HttpError(400, "invalid_request", "expected review revision is invalid");
		}
		const snapshot = await this.snapshots.getSnapshot();
		if (
			input.operationRevision !== undefined &&
			snapshot.operationRevision !== input.operationRevision
		) {
			throw new HttpError(
				409,
				"operation_changed",
				"Project changes changed; refresh before updating review state",
			);
		}
		const file = this.content.requireCurrentContent(snapshot, input.fileId, input.contentRevision);
		const result = await this.store.setReview(
			{
				fileId: file.id,
				path: file.path,
		...
		if (!result) {
			throw new HttpError(
				409,
				"review_changed",
				"Review state changed; refresh before updating the review mark",
			);
		}
		return {
	...
	async setReviews(input: SetReviewsRequest): Promise<SetReviewsResponse> {
		...
		const result = await this.store.setReviews(records);
		return { reviews: result.reviews, reviewRevision: result.revision };
	}
