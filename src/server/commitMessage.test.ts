...
describe("CodexCommitMessageService", () => {
	test("runs an isolated schema-constrained Luna request", async () => {
		...
		expect(command[command.indexOf("--model") + 1]).toBe(CODEX_COMMIT_MESSAGE_MODEL);
		expect(command).toContain(`model_reasoning_effort="${CODEX_COMMIT_MESSAGE_REASONING}"`);
	...
	test("rejects unavailable, malformed, and non-conventional output", async () => {
		...
		const invalid = new CodexCommitMessageService({
			executable: "codex",
			spawn: () => completedProcess(JSON.stringify({ message: "A plain commit subject" })),
		});
	...
	test("maps authentication and model failures without falling back", async () => {
		const loggedOut = new CodexCommitMessageService({
			executable: "codex",
			spawn: () => completedProcess("", "Not logged in. Run codex login.", 1),
		});
		...
		const missingModel = new CodexCommitMessageService({
			executable: "codex",
			spawn: () => completedProcess("", "Unknown model gpt-5.6-luna", 1),
		});
	...
	test("drains large stderr prompt echoes and retains failure details from the tail", async () => {
		...
		const loggedOut = new CodexCommitMessageService({
			executable: "codex",
			spawn: () => completedProcess("", `${promptEcho}\nNot logged in. Run codex login.\n`, 1),
		});
		await expect(loggedOut.generate("context")).rejects.toMatchObject({
			status: 503,
			code: "codex_login_required",
