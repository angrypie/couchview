...
describe("native bridge origin access", () => {
	...
	test("selects an installed provider by stable ID", async () => {
		...
		const session = remoteBridgeOriginAccessSession(provider.id, "https://relay.example.com", [
			provider,
		]);
		...
		expect(() =>
			remoteBridgeOriginAccessSession("missing-provider", "https://relay.example.com", [provider]),
