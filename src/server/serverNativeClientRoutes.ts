...
import { decodeSegment, json, normalizeOrigin, readJsonObject } from "./serverHttp.ts";
...
export async function handleNativeClientApi(
	...
	if (url.pathname === API_ROUTES.nativeClientPairings && request.method === "POST") {
		...
		return json(context.nativeClients.createPairing(normalizeOrigin(origin)), { status: 201 });
	}
