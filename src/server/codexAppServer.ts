...
function statusOf(value: unknown): CodexThreadStatus {
  ...
  return type === "active" || type === "idle" || type === "systemError"
    ? type
    : "notLoaded";
}
...
function requestError(error: RpcResponse["error"]): HttpError {
  ...
  if (/not logged|login|authenticat/i.test(message)) {
    return new HttpError(503, "codex_auth_required", "Codex is not logged in; run `codex login` and try again");
  }
  ...
  if (/already.*(loaded|running|owned)|in use|another.*client|active.*thread|one app-server|hold.*thread|process.*owns/i.test(message)) {
    return new HttpError(409, "codex_thread_in_use", "This Codex thread is currently owned by another client");
  }
...
export class CodexAppServerService {
  ...
  private handleLine(line: string): void {
    ...
    if (typeof record.method === "string") this.handleNotification(record as unknown as RpcNotification);
  }
  ...
  private handleNotification(notification: RpcNotification): void {
    ...
    this.emit(threadId, resolvedTurnId, {
      type: notification.method === "turn/completed"
        ? "completed"
        : notification.method === "error" ? "error" : "notification",
      threadId,
  ...
  private fail(error: unknown): void {
    ...
    const structured = reason instanceof HttpError
      ? reason
  ...
  private async write(value: string): Promise<void> {
    if (!this.process) throw new HttpError(503, "codex_unavailable", "Codex app-server is not running");
    try {
  ...
  private async ensureStarted(): Promise<void> {
    if (!this.capability.available || !this.executable) {
      throw new HttpError(503, "codex_unavailable", this.capability.reason ?? "Codex is unavailable");
    }
  ...
  async listThreads(root: string, cursor: string | null, limit = 40): Promise<{
  ...
  async startTurn(threadId: string, prompt: string): Promise<CodexTurnResponse> {
    ...
    if (existing && !existing.completed) {
      throw new HttpError(409, "codex_turn_active", "A Codex turn is already active for this thread");
    }
    ...
    try {
      ...
      if (!turnId) throw new HttpError(502, "codex_protocol_error", "Codex did not return a turn ID");
  ...
  events(
    ...
    if (!state || state.threadId !== threadId) return { events: [], found: false, unsubscribe() {} };
  ...
  async respondApproval(
    ...
    if (!approval || approval.threadId !== threadId) {
      throw new HttpError(404, "codex_approval_not_found", "Codex approval request is no longer pending");
    }
    this.approvals.delete(approvalId);
