...
const commandNames = ["serve", "restart", "completion", "bridge", "help"] as const;
...
export function parseServeArguments(
  ...
  const remoteBridgeP2pEnabled = booleanValue(
    parsed.values,
    "enable-remote-bridge-p2p",
  );
  const remoteBridgeP2pDisabled = booleanValue(
    parsed.values,
...
function parseBridgeArguments(args: string[]): CliInvocation {
  ...
  if (action === "terminal") {
    ...
    if (repositoryRoot !== undefined && !path.isAbsolute(repositoryRoot)) {
      throw new CliUsageError(
        "The bridge terminal repository path must be absolute.",
        "bridge",
      );
    }
    return {
  ...
  if (action === "claude") {
    ...
    if (repositoryRoot !== undefined && !path.isAbsolute(repositoryRoot)) {
      throw new CliUsageError(
        "The bridge claude repository path must be absolute.",
        "bridge",
      );
    }
    return {
  ...
  const suggestion = nearestValue(
    action ?? "",
    ["pair", "proxy", "codex", "terminal", "claude"],
  );
  throw new CliUsageError(
