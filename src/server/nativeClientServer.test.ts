...
): Promise<{ app: CouchviewApp; baseUrl: string }> {
...
describe("native client HTTP authority", () => {
	test("uses the validated browser origin when a tunnel reaches Couchview over HTTP", async () => {
		const publicOrigin = "https://review.example.com";
		const internalOrigin = "http://review.example.com";
		const { app } = await startFixture({ allowedOrigins: [publicOrigin] });
		const bootstrapResponse = await app.fetch(
			new Request(`${internalOrigin}${API_ROUTES.bootstrap}`),
		);
		const bootstrap = (await bootstrapResponse.json()) as { csrfToken: string };
		const pairingResponse = await app.fetch(
			new Request(`${internalOrigin}${API_ROUTES.nativeClientPairings}`, {
				method: "POST",
				headers: { origin: publicOrigin, [CSRF_HEADER]: bootstrap.csrfToken },
			}),
		);

		expect(pairingResponse.status).toBe(201);
		const pairing = (await pairingResponse.json()) as NativeClientPairingResponse;
		expect(pairing.baseUrl).toBe(publicOrigin);
		expect(new URL(pairing.deepLink).searchParams.get("baseUrl")).toBe(publicOrigin);
	});
