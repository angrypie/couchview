...
class FakeClientDataChannel {
	...
	readonly stateChanged = new FakeRtcEvent<["connecting" | "open" | "closing" | "closed"]>();
...
describe("native bridge client configuration", () => {
	...
	test("updates a profile without duplicating the OpenSSH include", async () => {
		...
		expect((await readFile(paths.sshConfigFile, "utf8")).match(/couchview_config/g)).toHaveLength(
			1,
		);
	});
...
describe("native bridge ProxyCommand", () => {
	test("composes a custom origin-access adapter with Couchview device authentication", async () => {
		...
		expect(
			Buffer.from(binarySent.buffer, binarySent.byteOffset, binarySent.byteLength).toString(),
		).toBe("ssh-client");
		...
		expect(requests[0]?.headers.get(REMOTE_BRIDGE_DEVICE_TOKEN_HEADER)).toBe(profile().deviceToken);
		expect(new URL(requests[0]!.url).pathname).toBe(API_ROUTES.remoteBridgeHostTickets);
		expect(new URL(socketUrl).pathname).toBe(API_ROUTES.remoteBridgeHostSocket);
		...
		expect(socketOptions).toMatchObject({
			protocols: [REMOTE_BRIDGE_PROTOCOL, expect.stringContaining("couchview-bridge-ticket.")],
		});
	...
	test("switches SSH traffic losslessly to the reliable WebRTC channel", async () => {
		...
		const runtime: Partial<RemoteBridgeClientRuntime> = {
			...
			createPeerConnection: () =>
				peer as unknown as ReturnType<RemoteBridgeClientRuntime["createPeerConnection"]>,
		};
		...
		for (
			let index = 0;
			index < 100 &&
			!activeSocket.sent.some(
				(value) => typeof value === "string" && value.includes("webrtc-offer"),
			);
			index += 1
		)
			await Bun.sleep(1);
		...
		expect(
			peer.channel.sent.map((value) => (typeof value === "string" ? value : value.toString())),
