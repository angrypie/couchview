...
function assertNonEmptyString(value: unknown, field: string, maximum = 10_000): asserts value is string {
...
export class GitRepository {
  ...
  private constructor(root: string, gitDirectory: string, indexPath: string, emptyTree: string) {
    this.root = root;
  ...
  static async open(candidate: string): Promise<GitRepository> {
    ...
    const emptyTreeResult = await runGit(root, ["hash-object", "-t", "tree", "--stdin"], {
  ...
  async diff(fileId: string): Promise<DiffResponse> {
    ...
    const parsed = patch
      ...
      : { header: [], hunks: [], additions: 0, deletions: 0, binary, truncated: false };
  ...
  async source(pathName: string, focusLine: number, context: number): Promise<SourcePreviewResponse> {
  ...
  async stage(input: StageFileRequest): Promise<StageFileResponse> {
    return this.withStageLock(async () => {
      ...
      if (before.operationRevision !== input.operationRevision) {
        throw new HttpError(409, "operation_changed", "Project changes changed; refresh before staging");
      }
      const file = this.requireFile(before, input.fileId);
      if (file.contentRevision !== input.contentRevision) {
        throw new HttpError(409, "content_changed", "File content changed; refresh before staging");
      }
      ...
      try {
        ...
        await this.stageExactPath(temporaryIndex, file.path, file.kind === "deleted");
        if (
          file.kind === "renamed" &&
          file.previousPath &&
          !locked.files.some(
          ...
          const previousExists = await lstat(this.resolveProjectPath(file.previousPath))
            .then(() => true)
            .catch((error) => {
        ...
        if (
          ...
          throw new HttpError(409, "operation_changed", "Project changes changed while staging; refresh first");
        }
        ...
        return {
          file: after.files.find((candidate) => candidate.id === input.fileId) ?? null,
  ...
  async reviewState(): Promise<ReviewStateResponse> {
    const [snapshot, state] = await Promise.all([this.getSnapshot(), this.store.snapshot()]);
  ...
  async setReview(input: SetReviewRequest): Promise<SetReviewResponse> {
    ...
    const snapshot = await this.getSnapshot();
    const file = this.requireCurrentContent(snapshot, input.fileId, input.contentRevision);
  ...
  async createComment(input: CreateCommentRequest): Promise<CommentResponse> {
    ...
    const oldStart =
      input.side === "new" ? undefined : input.oldStartLine ?? input.startLine;
    const oldEnd = input.side === "new" ? undefined : input.oldEndLine ?? input.endLine;
    const newStart =
      input.side === "old" ? undefined : input.newStartLine ?? input.startLine;
    ...
    let selected:
      | { hunk: (typeof currentDiff.hunks)[number]; lines: (typeof currentDiff.hunks)[number]["lines"] }
      | undefined;
    for (const hunk of currentDiff.hunks) {
      ...
      if (input.side === "mixed") {
        ...
        const contiguous = hunk.lines.slice(first, last + 1).every(
          (line) =>
            line.kind !== "metadata" &&
            (rangeContains(line.oldLine, oldStart, oldEnd) ||
              rangeContains(line.newLine, newStart, newEnd)),
    ...
    const excerpt = selected.lines.slice(0, 200).map((line) =>
      input.side === "mixed"
  ...
  startWatching(onChange: (operationRevision: string) => void): void {
    ...
    try {
      this.watcher = watch(this.root, { recursive: true }, (_event, filename) => {
        ...
        if (name.startsWith(`.git${path.sep}couch-review`) || name.startsWith(".git/couch-review")) return;
  ...
  private validateStatusIdentity(parsed: ReturnType<typeof parsePorcelainV2>): void {
    ...
    if ((!parsed.branch && !parsed.head && !parsed.unborn) || (parsed.unborn && !parsed.branch)) {
      throw new Error("Git status returned an incomplete repository identity");
    }
  }
  ...
  private async buildSnapshot(): Promise<Snapshot> {
    ...
    const operationRevision = sha256(
      ...
      indexResult.stdout,
      "\0",
      files.map((file) => `${file.id}:${file.contentRevision}:${file.indexStatus}${file.worktreeStatus}`).join("\0"),
    );
    return {
      repository,
      files,
      operationRevision,
      entries: new Map(parsed.entries.map((entry) => [sha256(entry.path).slice(0, 24), entry])),
    };
  }
  ...
  private requireCurrentContent(snapshot: Snapshot, fileId: string, revision: string): ChangeFile {
    const file = this.requireFile(snapshot, fileId);
  ...
  private async contentRevision(entry: ParsedStatusEntry, head: string | null): Promise<string> {
    const relativePath = entry.path;
    const absolutePath = this.resolveProjectPath(relativePath);
    const hash = createHash("sha256");
    const basePath = entry.kind === "renamed" && entry.previousPath
      ? entry.previousPath
      : relativePath;
    if (head) {
      const baseEntry = await runGit(this.root, ["ls-tree", "-z", head, "--", basePath]);
  ...
  private async removeIndexPathConflicts(
    ...
    await runGit(this.root, ["update-index", "--force-remove", "-z", "--stdin"], {
      env: { GIT_INDEX_FILE: indexPath },
  ...
  private async readWorkingFile(relativePath: string, maximum: number): Promise<WorkingFile> {
    const absolutePath = this.resolveProjectPath(relativePath);
    const metadata = await lstat(absolutePath).catch((error) => {
      if (["ENOENT", "ENOTDIR"].includes((error as NodeJS.ErrnoException).code ?? "")) {
  ...
  private async diffDeletedThenRecreated(
    ...
    const temporaryDirectory = await mkdtemp(path.join(tmpdir(), "couch-review-diff-"));
