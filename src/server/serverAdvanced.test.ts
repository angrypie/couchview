...
describe("Couchview HTTP advanced routes", () => {
	...
	test("guards single-file review updates with the repository operation revision", async () => {
		const app = await fixture();
		const before = await app.repository.changes();
		const file = before.files[0];
		if (!file) throw new Error("fixture file missing");
		await writeFile(path.join(app.repository.root, "second.ts"), "const second = true;\n");
		const after = await app.repository.changes();
		...
		expect(marked.status).toBe(200);
		expect(await marked.json()).toMatchObject({ operationRevision: after.operationRevision });
	});

	test("rejects voice undo after another review mutation changes the review revision", async () => {
		const app = await fixture();
		const changes = await app.repository.changes();
		const file = changes.files[0];
		if (!file) throw new Error("fixture file missing");
		const initial = await app.repository.reviewState();
		const voiceResult = await app.repository.setReview({
			fileId: file.id,
			contentRevision: file.contentRevision,
			operationRevision: changes.operationRevision,
			expectedReviewRevision: initial.revision,
			reviewed: true,
		});
		await app.repository.setReview({
			fileId: file.id,
			contentRevision: file.contentRevision,
			operationRevision: changes.operationRevision,
			expectedReviewRevision: voiceResult.reviewRevision,
			reviewed: false,
		});

		await expect(
			app.repository.setReview({
				fileId: file.id,
				contentRevision: file.contentRevision,
				operationRevision: changes.operationRevision,
				expectedReviewRevision: voiceResult.reviewRevision,
		...
		).rejects.toMatchObject({ status: 409, code: "review_changed" });
	});
