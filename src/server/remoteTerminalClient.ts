...
export interface RemoteTerminalClientRuntime {
  ...
  spawn(
    command: string[],
    options: RemoteInteractiveSpawnOptions,
  ): RemoteInteractiveProcess;
...
async function runRemoteInteractive(
  ...
  const profile = await resolveRemoteBridgeProfile(
    options.profileSelector,
    runtime.paths,
  );
  const repositoryRoot = options.repositoryRoot ?? profile.repositoryRoot;
  assertAbsoluteRepository(
    repositoryRoot,
    kind === "terminal" ? "terminal" : "Claude Code",
  );
  const sshExecutable = runtime.which("ssh");
  if (!sshExecutable) {
    throw new Error("OpenSSH is not available on this computer");
  }
  const command = kind === "terminal"
