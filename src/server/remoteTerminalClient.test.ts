...
describe("remote terminal command construction", () => {
  ...
  test("rejects relative repository paths before constructing SSH", () => {
    expect(() => remoteTerminalLaunchCommand(profile(), {
      sshExecutable: "ssh",
      repositoryRoot: "relative/project",
    })).toThrow("must be absolute");
    expect(() => remoteClaudeLaunchCommand(profile(), {
      sshExecutable: "ssh",
      repositoryRoot: "relative/project",
    })).toThrow("must be absolute");
  });
...
describe("remote terminal lifecycle", () => {
  ...
  test("fails before spawning when OpenSSH is unavailable", async () => {
    ...
    await expect(runRemoteTerminal({}, runtime)).rejects.toThrow(
      "OpenSSH is not available",
    );
    await expect(runRemoteClaude({}, runtime)).rejects.toThrow(
      "OpenSSH is not available",
    );
  });
});
