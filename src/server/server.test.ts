...
describe("Couchview HTTP security and routes", () => {
	...
	test("protects local voice resolution and preserves ordered array results", async () => {
		...
		const app = await fixture(undefined, undefined, undefined, undefined, undefined, voiceCommands);
		const body = JSON.stringify({ transcript: "open settings and artifacts" });
		const unauthorized = await app.fetch(
			request(API_ROUTES.voiceCommandResolve, {
				method: "POST",
		...
		expect(response.status).toBe(200);
		const resolved = (await response.json()) as ResolveVoiceCommandsResponse;
		expect(resolved.commands.map((command) => command.actionId)).toEqual([
			"navigate.settings",
			"navigate.artifacts",
		]);
		expect(resolved.confidence).toBe(0.82);
		expect(resolved.reasoning).toBe("'settings' -> open_settings; 'artifacts' -> open_artifacts");
	});
	...
	test("bootstraps, rejects foreign origins, and protects staging and committing", async () => {
		...
		expect(bootstrap.voiceCommands).toMatchObject({
			enabled: false,
			ready: false,
			state: "disabled",
			requiredFlags: ["--enable-speech", "--enable-voice-commands"],
		});
		const unavailableVoiceRetry = await app.fetch(
			request(API_ROUTES.voiceCommandRetry, {
				method: "POST",
				headers: {
		...
		expect(unavailableVoiceRetry.status).toBe(409);
