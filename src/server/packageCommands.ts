...
const ACTIVE_STATUSES = new Set<PackageRunSummary["status"]>([
  "running",
  "stopping",
]);
...
export class PackageCommandService {
  ...
  async discover(root: string): Promise<PackageScriptsResponse> {
    ...
    const packagePaths = decodeGitOutput(result.stdout)
      ...
      .filter(Boolean)
      .filter((packagePath) =>
        packagePath === "package.json" || packagePath.endsWith("/package.json")
      )
    ...
    const byPath = new Map(
      parsed.map((item) => [item.contract.packagePath, item]),
    );
  ...
  async start(
    ...
    if (
      [...this.storedRuns.values()].filter((run) =>
    ...
    const script = selectedPackage.scripts.find(
    ...
    try {
      run.process = this.spawnProcess(
        [executable, "run", script.name],
        {
  ...
  private async readPackage(
    ...
    const scripts = scriptsValue && typeof scriptsValue === "object" &&
  ...
  private async resolveRunner(
    ...
    for (const ancestor of this.ancestorDirectories(directory)) {
      const packagePath =
        ancestor === "." ? "package.json" : `${ancestor}/package.json`;
    ...
    for (const ancestor of this.ancestorDirectories(directory)) {
      const absolute =
        ancestor === "." ? root : path.join(root, ...ancestor.split("/"));
  ...
  private appendOutput(
    ...
    if (encoded.byteLength > this.maxOutputBytes) {
      sanitized = encoded
        .subarray(encoded.byteLength - this.maxOutputBytes)
        .toString("utf8");
  ...
  private emit(
    run: StoredRun,
    event: Exclude<PackageRunEvent, { type: "snapshot" }>,
  ): void {
