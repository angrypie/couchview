...
class FakeCodexProcess {
  ...
  async write(data: string): Promise<void> {
    for (const line of data.split("\n").filter(Boolean)) {
      ...
      if (message.method === "initialize") {
        ...
        const thread = this.thread(`new-${this.threads.size + 1}`, params.cwd, "new thread", "idle");
  ...
  private thread(id: string, cwd: unknown, preview: string, status: string): Record<string, unknown> {
    return {
      id,
...
describe("Codex app-server bridge", () => {
  ...
  test("creates, resumes, starts turns, streams events, and interrupts", async () => {
    ...
    const subscription = service.events(created.id, turn.turnId, 0, (event) => observed.push(event));
