...
function defaultRuntime(): RemoteCodexClientRuntime {
  return {
    ...
    selectRemotePort: () =>
      randomInt(MIN_REMOTE_CODEX_PORT, MAX_REMOTE_CODEX_PORT_EXCLUSIVE),
...
function remoteCodexStartupError(
  ...
  if (
    /permission denied|too many authentication failures|no supported authentication methods/i
      .test(diagnostic)
  ) {
    return new Error(
...
async function stopProcess(
  ...
  await Promise.race([
    ...
    runtime.wait(PROCESS_STOP_TIMEOUT_MS),
  ]);
  if (exited) return;
...
function assertCodexArguments(args: readonly string[]): void {
  ...
  for (const argument of args) {
    if (
      ...
      controlledOptions.some((option) =>
        argument === option || argument.startsWith(`${option}=`)
      )
    ) {
      throw new Error(
        `Codex option '${argument}' is controlled by the Couchview bridge`,
      );
    }
  }
}
...
export async function runRemoteCodex(
  ...
  const profile = await resolveRemoteCodexProfile(
    ...
    for (let attempt = 1; attempt <= MAX_CODEX_START_ATTEMPTS; attempt += 1) {
      ...
      if (tunnelExitCode !== null) {
        ...
        if (
          ...
          isPortConflictDiagnostic(diagnostic)
        ) {
          runtime.stderr(
