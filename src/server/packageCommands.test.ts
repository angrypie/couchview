...
describe("PackageCommandService", () => {
	test("discovers root and nested packages while excluding ignored and node_modules manifests", async () => {
		...
		git(root, "add", "package.json", "apps/web/package.json", "broken/package.json", ".gitignore");
	...
	test("uses the nearest lockfile and falls back to Bun", async () => {
		const root = await repositoryFixture();
		await put(root, "package.json", JSON.stringify({ scripts: { root: "echo root" } }));
		await put(root, "apps/api/package.json", JSON.stringify({ scripts: { api: "echo api" } }));
		...
		expect(result.packages.find((item) => item.directory === ".")?.runner).toBe("bun");
	...
	test("runs exact scripts, streams output, records failure, and rejects stale revisions", async () => {
		...
		const events: string[] = [];
		const subscription = service.subscribe("repository", started.id, (event) => {
		...
		const snapshotText = service
			.subscribe("repository", started.id, () => undefined)
		...
		expect(await terminalRun(service, "repository", failed.id)).toMatchObject({
			status: "failed",
			exitCode: 7,
		});

		await put(root, "package.json", JSON.stringify({ scripts: { ok: "echo changed" } }));
	...
	test("rejects duplicate active scripts and stops long-running processes", async () => {
		const root = await repositoryFixture();
		await put(root, "package.json", JSON.stringify({ scripts: { dev: "sleep 30" } }));
		git(root, "add", "package.json");
		const service = new PackageCommandService({
			resolveExecutable: () => process.execPath,
		});
		...
		expect(await service.start("repository", root, input).catch((caught) => caught)).toMatchObject({
			status: 409,
			code: "package_script_running",
		});
		expect(service.stop("repository", run.id).status).toBe("stopping");
		expect(await terminalRun(service, "repository", run.id)).toMatchObject({ status: "stopped" });
	...
	test("caps concurrency and retained output and records unavailable runners", async () => {
		...
		const first = await service.start("repository", root, inputFor("first"));
		expect(
			await service.start("repository", root, inputFor("second")).catch((caught) => caught),
		...
		const loudSnapshot = service.subscribe("repository", loud.id, () => undefined).snapshot;
		expect(loudResult.outputTruncated).toBe(true);
		expect(
			Buffer.byteLength(loudSnapshot.output.map((chunk) => chunk.text).join("")),
