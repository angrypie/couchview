...
import { cloudflareOriginAccessProvider } from "./cloudflareAccess.ts";
import {
	defaultRemoteBridgeExecutableCommand,
	normalizeRemoteBridgeOrigin,
	type RemoteBridgePaths,
	readRemoteBridgeConfig,
	resolveRemoteBridgePaths,
	storeRemoteBridgeProfile,
	validateRemoteBridgeProfile,
} from "./remoteBridgeConfig.ts";
import {
...
export interface RemoteBridgeClientRuntime {
	...
	createWebSocket(url: string, options: Bun.WebSocketOptions): WebSocket;
	createPeerConnection(iceServers: readonly { urls: string }[]): RTCPeerConnection;
...
function defaultRuntime(): RemoteBridgeClientRuntime {
	...
	return {
		...
		executableCommand: defaultRemoteBridgeExecutableCommand(),
		stdin: process.stdin,
...
export async function pairRemoteBridge(
	...
	const origin = normalizeRemoteBridgeOrigin(options.origin);
	...
	const profile = validateRemoteBridgeProfile(await response.json(), options.originAccess);
