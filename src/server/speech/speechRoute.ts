...
function requestedLanguage(url: URL): SpeechLanguageHint | undefined {
	const values = url.searchParams.getAll("language");
	if (values.length === 0) return undefined;
	if (values.length === 1 && isSpeechLanguageHint(values[0])) return values[0];
	throw new HttpError(
		400,
		"speech_language_invalid",
		"The requested speech language is not supported.",
	);
}
...
function declaredBodyBytes(request: Request): number | null {
	...
	return Number.isFinite(declaredLength) && declaredLength >= 0 ? declaredLength : null;
}
...
export async function handleSpeechApi(
	...
	const admission = speech.admitUpload(declaredBodyBytes(request));
	try {
		const bytes = await readBoundedBody(request);
		const wav = validatePcmWav(bytes);
		return json(
			await admission.transcribe(
				bytes,
				{
