...
export interface StoredReviewMutation {
	review: ReviewRecord;
	revision: number;
}
...
export class StateDatabase {
	...
	reviewState(repositoryId: string): StoredReviewState {
      ...
      `)
					.all({ repositoryId })
					.map(reviewFromRow);
				return { reviews, revision: this.stateRevision(repositoryId) ?? 0 };
			})
			.deferred();
	}
	...
	setReview(
		...
		record: ReviewRecord,
		expectedRevision?: number,
	): StoredReviewMutation | null {
		return this.database
			.transaction(() => {
				const revision = this.stateRevision(repositoryId);
				if (revision === null) {
					throw new Error(`Repository ${repositoryId} is not registered`);
				}
				if (expectedRevision !== undefined && revision !== expectedRevision) return null;
				this.database
					.query<
						unknown,
						{
      ...
      `)
				...
				return { review: structuredClone(record), revision: revision + 1 };
			})
			.immediate();
	}
