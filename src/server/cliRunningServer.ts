...
function parseInstanceResponse(value: unknown): InstanceResponse | null {
	...
	const valid =
		...
		typeof candidate.voiceCommandsEnabled === "boolean" &&
...
export async function registerWithRunningServer(
	...
	if (options.voiceCommandsEnabled && !rawInstance.voiceCommandsEnabled) {
		throw new Error(
			`Couchview is already using port ${options.port} with voice commands disabled; stop it or choose another port`,
		);
	}
