...
class FakeDataChannel implements TerminalDataChannel {
	...
	bufferedAmount = 0;
	readonly stateChanged = new FakeEvent<["open" | "closed" | "connecting" | "closing"]>();
...
function upgradeRequest(ticket: string): Request {
	return new Request("https://review.example.com/api/remote-bridge/socket", {
		headers: {
			host: "review.example.com",
			upgrade: "websocket",
			"sec-websocket-protocol": `${REMOTE_BRIDGE_PROTOCOL}, ${REMOTE_BRIDGE_TICKET_PREFIX}${ticket}`,
		},
	});
}

function serviceFixture(
	options: { p2pEnabled?: boolean; peer?: FakePeerConnection; now?: () => number } = {},
) {
...
function attach(
	...
	const ticket = service.issueTicket(deviceToken, { connectionId }, { host: "review.example.com" });
	const data = service.consumeUpgrade(upgradeRequest(ticket.ticket), {
		host: "review.example.com",
	});
...
describe("native remote bridge authorization", () => {
	...
	test("uses one-use pairing codes, hashed device credentials, and bound tickets", () => {
		...
		expect(() =>
			service.consumeUpgrade(upgradeRequest(issued.ticket), { host: "other.example.com" }),
		).toThrow(expect.objectContaining({ code: "remote_bridge_ticket_invalid" }));
		expect(() =>
			service.consumeUpgrade(upgradeRequest(issued.ticket), { host: "review.example.com" }),
		).toThrow(expect.objectContaining({ code: "remote_bridge_ticket_invalid" }));
		service.close();
		database.close();
	});
...
describe("native remote bridge transport", () => {
	test("pipes SSH bytes through WebSocket fallback and revokes an active device", () => {
		...
		expect(() =>
			service.consumeUpgrade(upgradeRequest(pending.ticket), { host: "review.example.com" }),
		).toThrow(expect.objectContaining({ code: "remote_bridge_ticket_invalid" }));
		service.close();
		database.close();
	});
	...
	test("hands off losslessly to reliable WebRTC and reconnects after active failure", async () => {
		...
		expect(Buffer.from(channel.sent[1] as Buffer).toString()).toBe("buffered-before-activation");
