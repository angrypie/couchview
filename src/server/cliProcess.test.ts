...
describe("Couchview CLI process", () => {
	...
	test("rejects unknown, action-specific, and removed completion arguments", async () => {
		const [unknownOption, wrongActionOption, completion, removedSpeechCommand] = await Promise.all([
			runCouchview(["serve", "--prot", "4173"]),
			runCouchview(["artifacts", "build", "app", "--force"]),
			runCouchview(["completion", "bash"]),
			runCouchview(["speech", "status"]),
		]);
		...
		expect(removedSpeechCommand.exitCode).toBe(2);
		expect(removedSpeechCommand.stderr).toContain("Unknown command 'speech'");
	});
