...
function profile(
  overrides: Partial<RemoteBridgeProfile> = {},
): RemoteBridgeProfile {
  return {
...
describe("remote Codex profile selection", () => {
  test("uses the only profile by default and accepts either its ID or SSH alias", async () => {
    ...
    expect((await resolveRemoteCodexProfile(null, stored.paths)).id).toBe(
      stored.profile.id,
    );
    expect((await resolveRemoteCodexProfile(
      stored.profile.id,
      stored.paths,
    )).sshAlias).toBe(stored.profile.sshAlias);
    expect((await resolveRemoteCodexProfile(
      stored.profile.sshAlias,
      stored.paths,
    )).id).toBe(stored.profile.id);
  });
  ...
  test("requires an explicit profile when more than one is paired", async () => {
    ...
    await expect(resolveRemoteCodexProfile(
      "missing-profile",
      stored.paths,
    )).rejects.toThrow("Available SSH hosts");
  });
});
...
describe("remote Codex command construction", () => {
  ...
  test("rejects arguments that could replace the managed endpoint or repository", () => {
    for (const argument of [
      ...
      expect(() => remoteCodexLaunchCommands(profile(), {
        sshExecutable: "ssh",
...
describe("remote Codex lifecycle", () => {
  test("waits for the remote app-server, launches the local TUI, and closes SSH", async () => {
    ...
    expect(commands[1]?.slice(3, 5)).toEqual([
      "--cd",
      stored.profile.repositoryRoot,
    ]);
  ...
  test("retries a local or remote forwarding port collision", async () => {
    ...
    expect(commands[1]).toContain(
      "127.0.0.1:43211:127.0.0.1:54322",
    );
