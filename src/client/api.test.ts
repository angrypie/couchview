...
describe("API client", () => {
	...
	test("adds a scoped speech language hint without changing ordinary dictation", async () => {
		...
		globalThis.fetch = ((input, init) => {
			...
			return Promise.resolve(
				Response.json({ durationMs: 500, inferenceMs: 5, language: "en", text: "review" }),
			);
		}) as typeof fetch;
		const controller = new AbortController();

		await api.transcribeSpeech(new Uint8Array([1]), "csrf", {
			language: "en",
			signal: controller.signal,
		});
		await api.transcribeSpeech(new Uint8Array([2]), "csrf");

		expect(requests).toEqual([
			{
				signal: controller.signal,
				url: "/api/speech/transcriptions?language=en",
			},
			{ signal: undefined, url: "/api/speech/transcriptions" },
		]);
	});
