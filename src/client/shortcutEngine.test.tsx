...
function commands(calls: CommandId[]): Record<CommandId, RuntimeCommand> {
  return Object.fromEntries(COMMAND_IDS.map((commandId) => [commandId, {
...
describe("shortcut engine", () => {
  test("normalizes Mod and explicit modifiers on Apple and non-Apple platforms", () => {
    ...
    expect(formatShortcut([{ key: "k", modifiers: ["mod", "shift"] }], true))
      .toBe("⌘⇧K");
    expect(formatShortcut([{ key: "k", modifiers: ["mod", "shift"] }], false))
      .toBe("Ctrl+Shift+K");
  });
