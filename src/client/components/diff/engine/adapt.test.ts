...
import { adaptFileDiff, fromDiffSide, reconstructUnifiedPatch, toDiffSide } from "./adapt.ts";
import type { ParsedDiffType } from "./types.ts";
...
describe("diff engine adapter", () => {
	...
	test("reuses the adapted diff for the same structured diff object", () => {
		const diff = fixtureDiff();
	...
	test("round-trips diff sides", () => {
		expect(fromDiffSide(toDiffSide("old"))).toBe("old");
		expect(fromDiffSide(toDiffSide("new"))).toBe("new");
	});
});
