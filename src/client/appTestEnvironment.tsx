...
// The community BottomSheet renders a Vaul drawer whose exit animation
// never completes under Happy DOM (no CSS animations), so closed sheets would
// stay mounted with scroll-lock and aria-hidden indefinitely. Substitute a
// no-animation portal so tests exercise app behavior around sheets; the real
// web drawer is covered by the Playwright suite.
function TestBottomSheet({
	...
	return createPortal(
		<div
			data-dynamic-sizing={String(enableDynamicSizing)}
			data-pan-down-to-close={String(enablePanDownToClose)}
			data-snap-points={snapPoints?.join(",")}
			data-testid="community-bottom-sheet"
		>
			{children}
		</div>,
		document.body,
