...
export function eventPresentation(event: CodexEvent): EventPresentation {
  if (event.type === "error") {
    ...
    return { kind: "text", text: typeof message === "string" ? message : "Codex reported an error." };
  }
  ...
  if (event.method === "item/started") {
    ...
    if (item?.type === "fileChange") return { kind: "file-change", text: "Codex is preparing file changes…" };
  ...
  if (event.method === "item/completed") {
    ...
    if (item?.type === "commandExecution") {
      return {
        ...
        status: item.status === "completed" ? "completed" : typeof item.status === "string" ? item.status : undefined,
      };
    }
    if (item?.type === "fileChange") {
      const status = typeof item.status === "string" ? item.status : "completed";
      return { kind: "file-change", text: status === "completed" ? "File changes completed." : `File changes ${status}.` };
    }
    return null;
  }
...
export function CodexCommentsPanel({
  ...
  const loadThreads = async (cursor: string | null = null, signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!cursor) {
        ...
        const available = response.threads.find((thread) => thread.id === stored && thread.status !== "active");
        setSelectedThreadId(available?.id ?? response.threads.find((thread) => thread.status !== "active")?.id ?? null);
      }
    } catch (error) {
  ...
  const respondApproval = async (
    ...
    try {
      ...
      const requestDecision = permissionRequest && (decision === "accept" || decision === "acceptForSession")
        ? {
  ...
  if (!capability.available) {
    return (
      <>
        <button aria-label="Close Codex thread picker" className="sheet-scrim codex-scrim" onClick={onClose} type="button" />
        <section aria-label="Send comments to Codex" aria-modal="true" className="bottom-sheet codex-sheet" role="dialog">
          <span className="sheet-grabber" />
          <header className="sheet-header">
            <div><h2 className="sheet-title">Send to Codex</h2><div className="repo-meta">Unavailable</div></div>
            <button aria-label="Close Codex thread picker" className="icon-button" onClick={onClose} type="button"><X size={19} /></button>
          </header>
          <div className="empty-state codex-empty-state"><CircleAlert className="state-icon" size={26} /><p className="state-copy">{capability.reason}</p></div>
          <footer className="sheet-footer"><button className="action-button secondary" onClick={onClose} type="button">Close</button></footer>
        </section>
      </>
    );
  }
  ...
  return (
    <>
      <button aria-label="Close Codex thread picker" className="sheet-scrim codex-scrim" onClick={onClose} type="button" />
      <section aria-label={view === "threads" ? "Send comments to Codex" : "Codex activity"} aria-modal="true" className="bottom-sheet codex-sheet" role="dialog">
        <span className="sheet-grabber" />
        <header className="sheet-header">
          <div>
            <h2 className="sheet-title">
              {view === "activity" && <button aria-label="Back to Codex threads" className="icon-button codex-back-button" onClick={() => setView("threads")} type="button"><ChevronLeft size={17} /></button>}
              {view === "threads" ? "Send to Codex" : "Codex activity"}
            </h2>
            <div className="repo-meta">
              {view === "threads" ? `${currentCommentCount} current comment${currentCommentCount === 1 ? "" : "s"}` : selectedThread?.preview || "Review comments"}
            </div>
          </div>
          <button aria-label="Close Codex activity" className="icon-button" onClick={onClose} type="button"><X size={19} /></button>
        </header>

        {view === "threads" ? (
          <>
            <div className="codex-toolbar">
              <button className="action-button secondary" disabled={threadBusy} onClick={() => void createAndSend()} type="button"><Plus size={15} /> New thread</button>
              <button aria-label="Refresh Codex threads" className="icon-button" disabled={loadingThreads || threadBusy} onClick={() => void loadThreads()} type="button"><RefreshCw className={loadingThreads ? "spinner" : ""} size={16} /></button>
            </div>
            <div className="comment-list codex-thread-list">
              {loadingThreads && threads.length === 0 ? (
                <div className="loading-state" style={{ minHeight: 150 }}><LoaderCircle className="state-icon spinner" size={24} /><p className="state-copy">Finding project threads…</p></div>
              ) : threads.length === 0 ? (
                <div className="empty-state codex-empty-state"><Send className="state-icon" size={26} /><p className="state-copy">No Codex threads exist for this project yet.</p></div>
              ) : (
                threads.map((thread) => {
                  const unavailable = thread.status === "active";
                  return (
                    <button className={`codex-thread-row ${selectedThreadId === thread.id ? "selected" : ""}`} disabled={unavailable || threadBusy} key={thread.id} onClick={() => setSelectedThreadId(thread.id)} title={unavailable ? "This thread is active in another Codex client. Hand it off there before sending." : undefined} type="button">
                      <span className="codex-thread-copy"><strong>{thread.preview || "Untitled Codex thread"}</strong><span>{formatTime(thread.recencyAt ?? thread.updatedAt)} · {statusLabel(thread)}</span></span>
                      {selectedThreadId === thread.id && <Check aria-label="Selected" size={16} />}
                    </button>
                  );
                })
              )}
              {nextCursor && <button className="text-button codex-load-more" disabled={loadingThreads} onClick={() => void loadThreads(nextCursor)} type="button">Load older threads</button>}
            </div>
            <footer className="sheet-footer codex-sheet-footer">
              <button className="action-button" disabled={!selectedThreadId || threadBusy || currentCommentCount === 0} onClick={() => selectedThreadId && void sendComments(selectedThreadId)} type="button">
                {threadBusy ? <LoaderCircle className="spinner" size={16} /> : <Send size={16} />} {currentCommentCount === 0 ? "No current comments" : "Send comments"}
              </button>
            </footer>
          </>
        ) : (
          <>
            <div className="codex-activity" aria-live="polite">
              {events.length === 0 && !eventError && <div className="loading-state" style={{ minHeight: 150 }}><LoaderCircle className="state-icon spinner" size={24} /><p className="state-copy">Waiting for Codex…</p></div>}
              {events.map((event) => {
                if (event.type === "approval") {
                  const busy = approvalBusy === event.approvalId;
                  const sessionApproval = event.approvalMethod?.includes("commandExecution")
                  ...
                  return (
                    <article className="codex-approval-card" key={`${event.approvalId}-${event.sequence}`}>
                      ...
                      <div className="codex-approval-actions">
                        <button className="action-button" disabled={busy} onClick={() => void respondApproval(event, "accept")} type="button">{busy ? <LoaderCircle className="spinner" size={14} /> : <Check size={14} />} Allow</button>
                        {sessionApproval && <button className="action-button secondary" disabled={busy} onClick={() => void respondApproval(event, "acceptForSession")} type="button">Allow for session</button>}
                        <button className="action-button secondary" disabled={busy} onClick={() => void respondApproval(event, "decline")} type="button">Deny</button>
                        <button className="text-button danger" disabled={busy} onClick={() => void respondApproval(event, "cancel")} type="button">Cancel turn</button>
                      </div>
                    </article>
                  );
                }
                ...
                if (presentation.kind === "output") {
                  return <pre className="codex-event codex-command-output" key={key}>{presentation.text}</pre>;
                }
                if (presentation.kind === "command") {
                  return (
                    <article className="codex-progress-card codex-command-card" key={key}>
                      <span>{presentation.status === "running" ? "Running command" : `Command ${presentation.status ?? "updated"}`}</span>
                      <code>{presentation.command}</code>
                    </article>
                  );
                }
                if (presentation.kind === "file-change") {
                  return <div className="codex-progress-card" key={key}>{presentation.text}</div>;
                }
                return <div className={`codex-event codex-event-${event.type}`} key={key}>{presentation.text}</div>;
              })}
              {eventError && <div className="codex-event codex-event-error"><CircleAlert size={15} /> {eventError}</div>}
            </div>
            <footer className="sheet-footer codex-sheet-footer">
              <button className="action-button secondary" disabled={!turn || Boolean(events.some((event) => event.type === "completed"))} onClick={() => void interrupt()} type="button"><Square size={14} /> Stop turn</button>
            </footer>
          </>
        )}
      </section>
