		...
		globalThis.fetch = (async (input, init) => {
			...
			if (url.pathname === "/api/bootstrap") {
				...
				return Response.json({
					...
					voiceCommands: {
						...
						reason: "Voice commands are disabled in this test.",
			...
			if (nestedPath === "files/review" && method === "GET") {
				return Response.json({
					...
					revision: fixture.reviewRevision,
				});
			}
			...
			if (nestedPath === "files/review" && method === "PUT") {
				...
				fixture.reviewRevision += reviews.length;
				return Response.json({ reviews, reviewRevision: fixture.reviewRevision });
			}
			...
			if (nestedPath === "files/first/review" && method === "PUT") {
				const input = body as { expectedReviewRevision?: number; reviewed: boolean };
				if (
					input.expectedReviewRevision !== undefined &&
					input.expectedReviewRevision !== fixture.reviewRevision
				) {
					return Response.json(
						{ error: { code: "review_changed", message: "Review state changed" } },
						{ status: 409 },
					);
				}
				fixture.files[0]!.reviewed = Boolean(input.reviewed);
				const review = {
					fileId: "first",
					path: "src/first.ts",
				...
				fixture.reviewRevision += 1;
				return Response.json({
					review,
					operationRevision: fixture.currentOperationRevision,
					reviewRevision: fixture.reviewRevision,
				});
			}
