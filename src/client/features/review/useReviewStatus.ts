...
import type { FileChange, ReviewStateResponse } from "../../../shared/contracts.ts";
...
export function useReviewStatus({
	...
	const refreshReviewState = useCallback(async () => {
		...
		if (!activeRepositoryId) return { reviews: [], revision: 0 } satisfies ReviewStateResponse;
		...
		reviewRevisionRef.current = state.revision;
		setReviewRevision(state.revision);
	...
	const setReviewed = useCallback(
		async (file: FileChange, reviewed: boolean, advance: boolean) => {
			...
			try {
				...
				reviewRevisionRef.current = response.reviewRevision;
				setReviewRevision(response.reviewRevision);
				if (advance && reviewed) {
	...
	const unreviewMultiple = useCallback(
		async (targets: FileChange[]) => {
			...
			try {
				const response = await api.setReviewedFiles(
					activeRepositoryId,
					{
	...
	return {
		bulkBusy,
		busy: busy || bulkBusy,
		getReviewRevision: () => reviewRevisionRef.current,
		refreshReviewState,
		reviewRevision,
		setReviewed,
		undoReview,
