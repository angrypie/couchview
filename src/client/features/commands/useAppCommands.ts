...
export function useAppCommands({
	...
	const commands = useMemo(() => {
		...
		return {
			...
			"file.toggleReviewed": command(
				...
				() => {
					if (activeFile) onReviewFile(activeFile);
				},
			),
	...
	const { pending } = useShortcutEngine({
		...
		paletteOpen,
		recording,
		reservedStrokes: voiceKeyboardActive ? VOICE_KEYBOARD_STROKES : undefined,
