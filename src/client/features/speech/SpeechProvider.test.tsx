...
function Harness({ first, second }: { first: SpeechTarget; second?: SpeechTarget }) {
	...
	return (
		<>
			...
			<button onClick={() => speech.stop(first)} type="button">
				Stop first
			</button>
			{second ? (
...
describe("SpeechProvider", () => {
	...
	test("ignores a push-to-talk release after the host limit already started transcription", async () => {
		const first = target("voice-command", "", { start: 0, end: 0 });
		...
		renderController({
			...
			first,
			maxDurationMs: 20,
			transcribe: (_body, _csrf, options) => {
		...
		fireEvent.click(screen.getByRole("button", { name: "Stop first" }));
		expect(requestSignal?.aborted).toBe(false);
		resolveResult({ durationMs: 500, inferenceMs: 1, language: null, text: "new" });
		await waitFor(() => expect(screen.getByTestId("speech-phase").textContent).toBe("idle"));
		expect(first.applied).toEqual(["new"]);
	});

	test("lets a voice-command target impose a shorter recording limit", async () => {
		const first = target("voice-command", "", { start: 0, end: 0 });
		first.maxDurationMs = 20;
		let stops = 0;
		const adapter: SpeechRecordingAdapter = {
			available: true,
			cancel: () => undefined,
		...
		renderController({ adapter, first, maxDurationMs: 300_000 });
		fireEvent.click(screen.getByRole("button", { name: "First mic" }));
		await waitFor(() => expect(stops).toBe(1));
	});

	test("rejects a detected language outside a target's language policy", async () => {
		const first = target("voice-command", "", { start: 0, end: 0 });
		first.language = "en";
		let transcriptionOptions: unknown;
		const adapter: SpeechRecordingAdapter = {
			available: true,
			cancel: () => undefined,
			start: async () => undefined,
		...
		fireEvent.click(screen.getByRole("button", { name: "First mic" }));
		await waitFor(() => expect(screen.getByTestId("speech-phase").textContent).toBe("error"));
		expect(screen.getByTestId("speech-error").textContent).toContain("English only");
		expect(screen.getByTestId("speech-error").textContent).toContain(
			"You said: “abrir definições”",
		);
		expect(transcriptionOptions).toMatchObject({ language: "en" });
		expect(first.applied).toEqual([]);
	});

	test("keeps automatic language detection for an ordinary dictation target", async () => {
		const first = target("dictation", "", { start: 0, end: 0 });
		let transcriptionOptions: unknown;
		const adapter: SpeechRecordingAdapter = {
			available: true,
			cancel: () => undefined,
		...
		renderController({
			...
			transcribe: async (...args) => {
				transcriptionOptions = args[2];
				return { durationMs: 500, inferenceMs: 5, language: "pt", text: "olá" };
			},
		});
		fireEvent.click(screen.getByRole("button", { name: "First mic" }));
		await waitFor(() => expect(screen.getByTestId("speech-phase").textContent).toBe("recording"));
		fireEvent.click(screen.getByRole("button", { name: "First mic" }));
		await waitFor(() => expect(screen.getByTestId("speech-phase").textContent).toBe("idle"));
		expect(transcriptionOptions).not.toHaveProperty("language");
		expect(first.applied).toEqual(["olá"]);
	});
