...
import { formatShortcut, shortcutStrokeFromEvent } from "./shortcutEngine.ts";
import {
  codeFontStack,
...
export function ProfileSettingsPage({
  ...
  const effectiveBindings = useMemo(
    () => effectiveKeybindings(draft.keyboard),
    [draft.keyboard],
  );
  ...
  const applyRecorded = (commandId: CommandId, binding: ShortcutSequence) => {
    ...
    const conflicts = keybindingConflicts(effectiveKeybindings(next.keyboard))
      .filter((conflict) => conflict.first === commandId || conflict.second === commandId);
    if (conflicts.length > 0) {
      const conflictingIds = [...new Set(conflicts.flatMap((conflict) =>
  ...
  const save = async () => {
    try {
      const parsed = parseSettingsProfileData(draft);
      await onSave(profile.id, name, parsed, profile.revision);
    } catch (error) {
  ...
  const diff = draft.typography.diff;
  const terminal = draft.typography.terminal;
  ...
  return (
    <section aria-label="Settings" className="settings-workspace">
      <header className="settings-toolbar">
        <button className="terminal-toolbar-button" onClick={close} type="button">
      ...
      <div className="settings-scroll">
        <div className="settings-intro settings-profile-intro">
          <div>
            <h1>Profiles</h1>
            <p>
              Profile contents are shared by this Couchview host. This browser remembers
              only which profile is selected.
            </p>
          </div>
          <div className="settings-profile-actions">
            <button className="text-button" onClick={() => void createProfile()} type="button">
              <Plus size={14} /> New
            </button>
            <button className="text-button" onClick={() => void duplicateProfile()} type="button">
        ...
        <div className="settings-profile-picker">
          ...
          <button
            ...
            onClick={() => setDraft(createDefaultSettingsProfileData())}
            type="button"
          >
        ...
        <div className="settings-grid settings-profile-grid">
          <section className="settings-card" aria-labelledby="appearance-settings-title">
            ...
            <FontFamilyPicker
              ...
              onChange={(fontFamily) => updateDraft((next) => {
            ...
            <TypographySlider
              ...
              id="diff-font-size"
              label="Font size"
              onChange={(fontSize) => updateDraft((next) => {
            ...
            >
              <div className="diff-preview-lines">
                <span><i>12</i><b>−</b> const layout = legacyBrowser;</span>
                <span><i>12</i><b>+</b> const layout = activeProfile;</span>
              </div>
              <span className="sr-only" data-testid="diff-column-ruler">
                Diff column ruler through 80
              </span>
            </div>
            <h3><SquareTerminal size={15} /> Terminal</h3>
            <FontFamilyPicker
          ...
          <section className="settings-card" aria-labelledby="display-settings-title">
            <header className="settings-card-header">
              <div className="settings-card-heading">
                <span className="settings-card-icon"><Settings2 size={18} /></span>
                <div>
            ...
            <label className="settings-toggle-row">
              <span><strong>Line numbers</strong><small>Show source line numbers in diffs.</small></span>
              <input
                checked={draft.display.lineNumbersVisible}
            ...
            <label className="settings-toggle-row">
              <span><strong>Wrap long lines</strong><small>Keep long diff lines visible without horizontal scrolling.</small></span>
              <input
                checked={draft.display.lineWrapEnabled}
                onChange={(event) => updateDraft((next) => {
          ...
          <section className="settings-card settings-keyboard-card" aria-labelledby="keyboard-settings-title">
            <header className="settings-card-header">
              <div className="settings-card-heading">
                <span className="settings-card-icon"><Keyboard size={18} /></span>
                <div>
            ...
            <fieldset className="settings-fieldset keyboard-layout-picker">
              ...
              <label>
                <input
                  checked={draft.keyboard.layout === "qwerty"}
                  name="keyboard-layout"
                  onChange={() => updateDraft((next) => {
                    next.keyboard.layout = "qwerty";
                    return next;
                  })}
                  type="radio"
                />
              ...
              <label>
                <input
                  checked={draft.keyboard.layout === "dvorak"}
                  name="keyboard-layout"
                  onChange={() => updateDraft((next) => {
                    next.keyboard.layout = "dvorak";
                    return next;
                  })}
                  type="radio"
                />
              ...
              {COMMAND_IDS.map((commandId) => {
                ...
                return (
                  <div className="keybinding-row" key={commandId}>
                    ...
                    <kbd>{isRecording
                      ? recorded.length > 0 ? formatShortcut(recorded) : "Type shortcut…"
                      : formatShortcut(effectiveBindings[commandId])}</kbd>
                    <button
                      data-shortcut-capture={isRecording ? "true" : undefined}
                      onClick={() => {
                        recordedRef.current = [];
                        setRecorded([]);
                        setRecordingId(isRecording ? null : commandId);
                      }}
                      type="button"
                    >
